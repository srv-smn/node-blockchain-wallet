...
router.post('/me/send', auth, verify, async (req, res) => {
  try {
    ...
    const sendersBalance = await token.methods
      .balanceOf(req.user.account)
      .call();
    const sendersBalanceStandart = await web3.utils.fromWei(
      sendersBalance,
      'ether'
    );

    const receiverBalance = await token.methods
      .balanceOf(recipient.account)
      .call();
    const receiverBalanceStandart = await web3.utils.fromWei(
      receiverBalance,
      'ether'
    );
...
router.get('/transaction/:id', auth, async (req, res) => {
  ...
  try {
    ...
    const task = await Transaction.findOne({
      $and: [
        { _id },
...
router.get('/transactions', auth, verify, async (req, res) => {
  try {
    // admin will be able to see all transaction
    if (req.user.admin) {
